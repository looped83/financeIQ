...
* 🔶 **Phase 3 (started)** — Tabs migrated to components one at a time.
  ...
  - All nine previewable standalone (`npm run dev`, open `/src/dev/{transactions,overview,categories,yearly,monthly,outliers,forecast,deepdive,compare}-preview.html`), all verified with Playwright against the *built* output — not just unit tests.
  - Not wired into `index.html` yet; remaining tab (Empfehlungen) still to come.

...
- **Phase 3 (komplett, alle 11 Tabs fertig, inkl. nachträglich ergänztem Zeitverlauf-Tab):** Tabs einzeln auf Komponenten umstellen. Als Rendering-Library kommt **lit-html** zum Einsatz (~5kb, Template-Literal-basiert, kein virtuelles DOM, keine transitiven Dependencies) — passt zum bisherigen "Funktion baut Markup"-Stil, aber mit gezieltem statt komplettem Re-Render und echten Event-Listenern statt `onclick="..."`-Strings.
  - `src/features/timeline/` — **nachträglich ergänzt**: beim Vorbereiten von Phase 5 fiel auf, dass "Zeitverlauf" (Tab 1, `renderTimeline`) in Phase 3 versehentlich übersprungen worden war (`timelineView`/`setTimelineView` existierten im Phase-2-Store bereits, aber keine Komponente konsumierte sie). `selectors.ts` (9 Tests) + `TimelineView.ts`: 5 Charts — Hauptchart mit Täglich/Monatlich/Quartalsweise-Umschaltung (inkl. gleitendem Durchschnitt über die *kumulierte* Serie, nicht über die Deltas — 1:1 wie im Original), monatlicher Netto-Cashflow, Dividenden-Verlauf, Käufe/Verkäufe, getrennt kumulierte Einnahmen/Ausgaben. Die Tagesansicht nutzt eine Chart.js-Zeitachse, dafür wurde `chartjs-adapter-date-fns` (+ `date-fns`) als echte npm-Dependency ergänzt und einmalig in `chartManager.ts` registriert (vorher nur als CDN-Script im Original)
  - `src/features/recommendations/` — `selectors.ts` (`computeRecommendations(a)`, 16 Tests) + `RecommendationsView.ts`. Letzter noch fehlender Tab, 1:1 aus `renderRecommendations()` portiert: 15 Regeln (Cashflow, Sparquote, Investitionsrate, Dividenden inkl. Klumpenrisiko, Gebühren, Steuern/Freistellungsauftrag, Großausgaben, Fixkosten/Abos, Volatilität, Portfolio-Diversifikation, Kartenanteil, Notfallrücklage, Ausgaben-/Einnahmen-Trends, Händler-Konzentration, Wochenendausgaben), nach Priorität sortiert. Keine Charts, kein Store-State über die reine Analyse hinaus — wie im Original
  - `src/features/compare/` — `selectors.ts` (9 exportierte Funktionen, 15 Tests) + `CompareView.ts`. Zeigt Upload-Screen für die zweite CSV, danach Dashboard mit 3-Spalten-KPI-Grid inkl. Delta-Spalte, metrik-umschaltbarem Monats-Chart (Einnahmen/Ausgaben/Netto-Cashflow/Investitionen), Netto-Delta-Chart, Kategorie-Delta-Chart, Auto-Erkenntnissen und vollständiger Delta-Tabelle. `buildMonthAlign` richtet die Monate beider Dateien nach Kalendermonat (01–12) aus, unabhängig vom Jahr — ein 2024er-File vergleicht sich so sauber gegen ein 2025er-File. Bewusst 1:1 erhalten: die "neue Ausgabenkategorie"-Erkennung nutzt `!a1.expCat[c]` (Falsy-Check wie im Original), nicht `in`/`hasOwnProperty` (Kommentar im Code dokumentiert den Unterschied). An den Store über `compare.analysis`/`compare.metric`/`loadCompareFile`/`setCompareMetric`/`resetCompare` angebunden; die zweite CSV wird direkt in der Komponente eingelesen (`parseCSV`+`analyze` im `@change`-Handler), da der Upload-Input nur bedingt innerhalb des Vergleich-Tabs selbst existiert, nicht als statisches Seiten-Element
  - `src/features/deepdive/` — **der größte Tab**: `selectors.ts` (10 exportierte Funktionen, 18 Tests) + `DeepDiveView.ts` (4 Chart.js-Charts). `buildMonthlySnapshots(a)` teilt die hochgeladene CSV nach Monat auf und analysiert jeden Monat isoliert neu — 1:1 wie im Original. Dabei fällt auf (dokumentiert, nicht "repariert"): weil jede Monats-Analyse nur Zeilen EINES Monats enthält, ist `analysis.subscriptions` in jedem Snapshot immer leer (wiederkehrende Ausgaben brauchen per Definition 2+ verschiedene Monate) — betrifft auch die "Mehr wiederkehrende Ausgaben"-Trend-Erkennung, die dadurch nie auslöst. Bereits im Original so, keine neue Einschränkung. Der Monats-Dropdown bei "Veränderungen zum Vormonat" ist an `deepDiveSelectedMonth`/`setDeepDiveMonth` im Store angebunden und defaultet beim ersten Rendern auf den letzten Monat (löst dafür einmalig einen zusätzlichen Render-Durchlauf aus — sauber, kein Endlos-Loop, siehe Kommentar in `mountDeepDiveView`)
  - `src/features/outliers/` — `selectors.ts` (6 Tests) + `OutliersView.ts` (keine Charts): KPIs, Risikoampel, wiederkehrende Ausgaben (nutzt `getRecurringExpenses` aus dem shared Modul), Z-Score-Ausreißer-Tabelle mit Kritisch/Erhöht/Auffällig-Einstufung
  - `src/features/forecast/` — `selectors.ts` (6 Tests) + `ForecastView.ts`: Lineartrend-Cashflow-Prognose mit 95%-Konfidenzband, nutzt `linReg` direkt aus dem Phase-1-Domain-Layer. Die 3/6/12-Monate-Umschaltung ist über `forecastMonths`/`setForecastMonths` an den Phase-2-Store angebunden — echter interaktiver State, keine Demo-Attrappe (per Playwright verifiziert: Klick auf "12 Monate" aktualisiert Button-Zustand und Chart)
  ...
  - `src/dev/{transactions,overview,categories,yearly,monthly,outliers,forecast,deepdive,compare,recommendations,timeline}-preview.{html,ts}` — je eine eigenständige Vite-Entry-Seite zum echten Ausprobieren (CSV hochladen, live interagieren), ohne `index.html` anzurühren; `vite.config.ts` hat jetzt 12 Rollup-Inputs, `dist/index.html` bleibt dabei weiterhin byte-identisch (verifiziert)

...
- **Phase 3 (gestartet, Transaktionen + Übersicht + Kategorien + Jahre + Monate + Ausreißer + Prognose fertig):** Tabs einzeln auf Komponenten umstellen. Als Rendering-Library kommt **lit-html** zum Einsatz (~5kb, Template-Literal-basiert, kein virtuelles DOM, keine transitiven Dependencies) — passt zum bisherigen "Funktion baut Markup"-Stil, aber mit gezieltem statt komplettem Re-Render und echten Event-Listenern statt `onclick="..."`-Strings.
  ...
  - `src/dev/{transactions,overview,categories,yearly,monthly,outliers,forecast}-preview.{html,ts}` — je eine eigenständige Vite-Entry-Seite zum echten Ausprobieren (CSV hochladen, live interagieren), ohne `index.html` anzurühren; `vite.config.ts` hat jetzt 8 Rollup-Inputs, `dist/index.html` bleibt dabei weiterhin byte-identisch (verifiziert)
  ...
  - Noch offen: restliche Tabs (Deep-Dive/Vergleich → Empfehlungen)
